...
begin
  BeginTest('documented example goldens');
...
procedure CheckScientificExampleGoldens;
var
  AttemptedCount: Integer;
...
begin
  BeginTest('scientific example goldens');
...
procedure CheckPartialFailure;
var
  AttemptedCount: Integer;
  PartialProject: TDocProject;
begin
  BeginTest('partial failure');
...
procedure CheckDependencyGraph;
var
  AttemptedCount: Integer;
...
begin
  ...
  try
    ...
    DeleteTree('build/dependency-test-docs');
...
procedure CheckTypeRelationships;
var
  AttemptedCount: Integer;
...
begin
  ...
  try
    ...
    DeleteTree('build/relationship-test-docs');
...
begin
  ...
  try
    ...
    DeleteTree('build/math-test-docs');
...
procedure CheckCommentDialects;
var
  AttemptedCount: Integer;
...
procedure CheckSourceDiscovery;
var
  AttemptedCount: Integer;
...
begin
  ...
  LazarusConfiguration := LoadLazarusConfiguration(
    'tests/fixtures/lazarus/separate-target/SeparateTarget.lpi', '', nil);
  try
    Check((LazarusConfiguration.CompilerOptions.TargetOS = 'linux') and
      (LazarusConfiguration.CompilerOptions.TargetCPU = 'aarch64'),
      'separate-token Lazarus -T and -P options should set the target');
  finally
    LazarusConfiguration.Free;
  end;
  ...
  try
    ...
    Check(Pos('href=', string(RenderInlineMarkdown(
      '[unsafe](JaVaScRiPt:alert(1))'))) = 0,
      'HTML Markdown rendering should reject case-varied active schemes');
    Check(Pos('href=', string(RenderInlineMarkdown(
      '[unsafe](java' + #9 + 'script:alert(1))'))) = 0,
      'HTML Markdown rendering should reject control-character scheme splits');
    Check(Pos('href=', string(RenderInlineMarkdown(
      '[unsafe](data:text/html,<script>alert(1)</script>)'))) = 0,
      'HTML Markdown rendering should reject data: link targets');
    Check(Pos('href=', string(RenderInlineMarkdown(
      '[unsafe](vbscript:msgbox(1))'))) = 0,
      'HTML Markdown rendering should reject vbscript: link targets');
    Check(Pos('href=', string(RenderInlineMarkdown(
      '[unsafe](ftp://example.com/file)'))) = 0,
      'HTML Markdown rendering should allow only documented link schemes');
    Check(Pos('href="https://example.com/guide"',
      string(RenderInlineMarkdown('[guide](https://example.com/guide)'))) > 0,
      'HTML Markdown rendering should keep HTTPS links');
    Check(Pos('href="mailto:dev@example.com"',
      string(RenderInlineMarkdown('[mail](mailto:dev@example.com)'))) > 0,
      'HTML Markdown rendering should keep mailto links');
    Check(Pos('href="#scope"',
      string(RenderInlineMarkdown('[jump](#scope)'))) > 0,
      'HTML Markdown rendering should keep fragment links');
    Check(Pos('href="docs/guide.md"',
      string(RenderInlineMarkdown('[guide](docs/guide.md)'))) > 0,
      'HTML Markdown rendering should keep relative links');
...
begin
  Failures := 0;
  RunCase('navigation', @RunNavigationTests);
  RunCase('symbol index and theme', @RunSymbolIndexAndThemeTests);
  RunCase('model, renderers, compiler, and Lazarus', @RunTests);
  RunCase('validation', @RunValidationTests);
  RunCase('incremental', @RunIncrementalTests);
  RunCase('documented example goldens', @CheckDocumentedExampleGoldens);
  RunCase('scientific example goldens', @CheckScientificExampleGoldens);
  RunCase('dependency diagram', @CheckDependencyGraph);
  RunCase('type relationships', @CheckTypeRelationships);
  ...
  if Failures > 0 then
  begin
    WriteLn(StdErr, Failures, ' test case(s) failed.');
    Halt(1);
  end;
